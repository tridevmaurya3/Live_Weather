...
public final class HeroGlPortableStormRenderer {
    ...
    private static final String FS=String.join("\n",
            ...
            " float anchor=0.17+texRnd(macro*0.041+0.37,0.63)*0.66;float y0=0.07+texRnd(macro*0.029+0.19,0.77)*0.14;float y1=0.63+texRnd(macro*0.053+0.51,0.21)*0.18;float drift=(texRnd(seed,0.44)-0.5)*(0.10+uWind*0.07);float mainBolt=boltLine(p,anchor,seed*7.0+1.3,y0,y1,0.0020+storm*0.0020,drift);",
            " float fork1=0.0;float fork2=0.0;float companion=0.0;if(detail>0.72){float fork1Gate=step(0.36,texRnd(macro*0.067+0.11,0.52));fork1=boltLine(p,anchor+0.012,seed*11.0+4.7,y0+0.18,y1-0.03,0.00144,0.12+drift*0.42)*fork1Gate;if(detail>0.90){float fork2Gate=step(0.68,texRnd(macro*0.083+0.61,0.26));fork2=boltLine(p,anchor-0.010,seed*13.0+2.1,y0+0.28,y1-0.10,0.00128,-0.10+drift*0.35)*fork2Gate;}float companionGate=step(0.80,texRnd(macro*0.101+0.44,0.18))*smoothstep(0.58,0.94,storm);companion=boltLine(p,anchor+(texRnd(seed,0.91)-0.5)*0.28,seed*17.0+3.0,y0+0.05,y1-0.12,0.00140,-drift*0.45)*companionGate;}",
