...
/** Shared storm/lightning pass with governor-controlled secondary branch detail. */
public final class HeroGlPortableStormRenderer {
    ...
    private static final String FS=String.join("\n",
            ...
            "  float micro=sin(t*43.0+seed*5.7)*0.0075+sin(t*89.0+seed*2.3)*0.0042+sin(t*151.0+seed)*0.0020;",
            "  float bend=sin(t*7.2+seed*1.9)*0.011*t;float x=anchor+n*0.112+micro+bend+drift*t;float aspect=uResolution.x/max(1.0,uResolution.y);",
            "  float d=abs((p.x-x)*aspect);float core=exp(-d/max(0.00055,width));float glow=exp(-d/max(0.0030,width*5.9))*0.27;return (core+glow)*inside;",
            "}",
            "void main(){",
            ...
            "  float rainLoad=clamp(uRain,0.0,1.0);float darkMask=smoothstep(0.37,0.68,cloudN)*upper*cloudMass;float baseVeil=upper*cloudMass*storm*(0.010+rainLoad*0.018);",
            "  float darkAlpha=(0.026+storm*0.098+rainLoad*0.030)*darkMask+baseVeil;",
            "  float macro=floor(uTime/7.4);float phase=mod(uTime,7.4);float seed=texRnd(macro*0.037+0.13,0.31);",
            "  float strikeGate=step(0.34-storm*0.20,seed);float start=0.48+texRnd(macro*0.047+0.23,0.73)*4.75;float local=phase-start;",
            "  float p1=0.0;if(local>=0.0&&local<0.070)p1=1.0-local/0.070;",
            "  float p2=0.0;if(local>=0.115&&local<0.205)p2=(1.0-(local-0.115)/0.090)*0.48;",
            "  float p3=0.0;if(local>=0.255&&local<0.335)p3=(1.0-(local-0.255)/0.080)*0.16;",
            "  float pulse=(p1+p2+p3)*storm*uElectricalEnabled*strikeGate;",
            "  float anchor=0.17+texRnd(macro*0.041+0.37,0.63)*0.66;float y0=0.07+texRnd(macro*0.029+0.19,0.77)*0.14;float y1=0.63+texRnd(macro*0.053+0.51,0.21)*0.18;",
            ...
            "  float fork1=0.0;float fork2=0.0;float companion=0.0;if(detail>0.72){",
            "    float fork1Gate=step(0.36,texRnd(macro*0.067+0.11,0.52));fork1=boltLine(p,anchor+0.012,seed*11.0+4.7,y0+0.18,y1-0.03,0.00072,0.12+drift*0.42)*fork1Gate;",
            "    if(detail>0.90){float fork2Gate=step(0.68,texRnd(macro*0.083+0.61,0.26));fork2=boltLine(p,anchor-0.010,seed*13.0+2.1,y0+0.28,y1-0.10,0.00064,-0.10+drift*0.35)*fork2Gate;}",
            "    float companionGate=step(0.78,texRnd(macro*0.101+0.44,0.18))*smoothstep(0.58,0.94,storm);",
            "    companion=boltLine(p,anchor+(texRnd(seed,0.91)-0.5)*0.28,seed*17.0+3.0,y0+0.05,y1-0.12,0.00070,-drift*0.45)*companionGate;",
            "  }",
            "  float bolt=(mainBolt+fork1*0.56+fork2*0.40+companion*0.46)*pulse;",
            "  float horizontalGlow=exp(-abs(p.x-anchor)*(4.5+storm*1.5));float verticalGlow=1.0-smoothstep(0.50,0.94,p.y);",
            "  float cloudGlowMask=mix(0.30,0.92,smoothstep(0.28,0.76,cloudN));float chargeCore=exp(-abs(p.x-anchor)*(9.0+storm*4.0))*upper;",
            "  float localGlow=horizontalGlow*verticalGlow*cloudGlowMask*pulse*(0.12+cloudMass*0.25);",
            "  float cloudCharge=chargeCore*cloudGlowMask*pulse*(0.11+cloudMass*0.20);",
            "  float broadFlash=upper*cloudGlowMask*pulse*(0.017+storm*0.052)*(0.48+cloudMass*0.52);",
            "  float horizonFlash=smoothstep(0.62,0.94,p.y)*pulse*(0.010+rainLoad*0.020)*horizontalGlow;",
            ...
            "  alpha=1.0-(1.0-alpha)*(1.0-darkness);color+=vec3(0.50,0.63,0.89)*localGlow;alpha=1.0-(1.0-alpha)*(1.0-clamp(localGlow*0.50,0.0,0.32));",
            "  color+=vec3(0.62,0.73,0.98)*cloudCharge;alpha=1.0-(1.0-alpha)*(1.0-clamp(cloudCharge*0.42,0.0,0.22));",
            "  color+=vec3(0.69,0.79,0.98)*broadFlash+vec3(0.43,0.54,0.76)*horizonFlash;",
            "  alpha=1.0-(1.0-alpha)*(1.0-clamp(broadFlash+horizonFlash,0.0,0.14));color+=vec3(0.92,0.96,1.0)*bolt;",
