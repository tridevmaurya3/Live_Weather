...
public final class HeroGlAnalyticCloudRenderer {
    ...
    private static final String FRAGMENT_SHADER = String.join("\n",
            ...
            "  return a+b*0.72+c*0.58;",
            "}",
            "float softMask(float field,float density,float layerBias){",
            "  float threshold=mix(0.72,0.38,clamp(density+layerBias,0.0,1.0));",
            "  return smoothstep(threshold,threshold+0.46,field);",
            "}",
            "void main(){",
            "  vec2 p=vec2(vUv.x,1.0-vUv.y);",
            "  if(uCloud<=0.004){gl_FragColor=vec4(0.0);return;}",
            "  float aspect=uResolution.x/max(1.0,uResolution.y);",
            "  p.x=fract((p.x-0.5)*(aspect/" + REFERENCE_ASPECT + ")+0.5);",
            ...
            "  float ceilingBand=smoothstep(0.01,0.16,p.y)*(1.0-smoothstep(0.62,0.86,p.y));",
            "  float overcastAmount=smoothstep(0.70,0.91,uCloud);",
            "  float ceilingStrength=max(uCeiling,overcastAmount*(0.58+uDensity*0.34));",
            "  float ceiling=ceilingBand*ceilingNoise*ceilingStrength;",
            "",
            ...
            "",
            "  float farOuter=softMask(farField,max(0.0,uDensity-0.08),0.0)*uFar;",
            "  float farEdge=max(0.0,farOuter-farM);",
            ...
            "  float ceilA=clamp(ceiling*(0.34+ceilingStrength*0.46),0.0,0.84);",
