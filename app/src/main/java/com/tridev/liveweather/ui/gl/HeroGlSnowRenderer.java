...
/** Depth-aware snowfall with governor-controlled secondary detail. */
public final class HeroGlSnowRenderer {
    ...
    private static final String FS=String.join("\n",
            ...
            "  vec2 q=p*vec2(cols,rows);q.x-=uTime*drift;q.y-=uTime*speed;vec2 id=floor(q);",
            "  float r1=cellRnd(id,seed);float r2=cellRnd(id,seed+7.3);vec2 f=fract(q)-0.5;",
            "  f.x+=(r1-0.5)*0.54;f.y+=(r2-0.5)*0.24;f.x+=sin((f.y+r1)*6.28318+uTime*(0.45+r2*0.55))*wobble;",
            "  float d=length(f);float soft=1.0-smoothstep(radius,radius*1.85,d);return soft*step(1.0-density,r2);",
            "}",
            "void main(){",
            "  vec2 p=vec2(vUv.x,1.0-vUv.y);float snow=clamp(uSnow,0.0,1.0);",
            "  if(snow<0.004){gl_FragColor=vec4(0.0);return;}",
            "  float detail=clamp(uDetail,0.5,1.0);float side=sin(uWindDir);float drift=side*(0.015+uWind*0.085);",
            ...
            "  float near=0.0;if(detail>0.62){near=flake(p+vec2(0.47,0.21),13.0,10.0,0.118+snow*0.052,15.4,0.105,0.15+snow*0.42,drift*1.10,0.070);}",
            ...
            "  vec3 color=flakeColor*flakeAlpha+vec3(0.70,0.78,0.84)*depthMist;",
            "  float alpha=1.0-(1.0-flakeAlpha)*(1.0-depthMist);",
            "  gl_FragColor=vec4(clamp(color,0.0,1.0),clamp(alpha,0.0,0.90));",
            "}");

    private final FloatBuffer quad;
    private int program,noiseTexture,aPosition,uNoise,uTime,uSnow,uWind,uWindDir,uSceneLight,uVisibility,uDetail;
    private long startNanos;
    private volatile float detailScale=1f;
    @Nullable private volatile GlSceneSnapshot snapshot;

    public HeroGlSnowRenderer(){ByteBuffer bytes=ByteBuffer.allocateDirect(QUAD.length*4).order(ByteOrder.nativeOrder());quad=bytes.asFloatBuffer();quad.put(QUAD).position(0);}
    ...
    public void onSurfaceCreated(){
        program=createProgram(VS,FS);noiseTexture=GlDeterministicTextureFactory.createCloudNoiseTexture();
        aPosition=GLES20.glGetAttribLocation(program,"aPosition");uNoise=u("uNoise");uTime=u("uTime");uSnow=u("uSnow");uWind=u("uWind");uWindDir=u("uWindDir");
        uSceneLight=u("uSceneLight");uVisibility=u("uVisibility");uDetail=u("uDetail");startNanos=System.nanoTime();
    ...
    public void drawFrame(){
        GlSceneSnapshot s=snapshot;if(program==0||noiseTexture==0||s==null||s.snowIntensity<=0.003f)return;
        ...
        GLES20.glUniform1f(uTime,(System.nanoTime()-startNanos)/1_000_000_000f);GLES20.glUniform1f(uSnow,s.snowIntensity);GLES20.glUniform1f(uWind,s.windStrength);
        GLES20.glUniform1f(uWindDir,s.windDirectionRadians);GLES20.glUniform1f(uSceneLight,s.sceneLight);GLES20.glUniform1f(uVisibility,s.visibilityFactor);GLES20.glUniform1f(uDetail,detailScale);
