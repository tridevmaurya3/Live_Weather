...
public final class HeroGlSnowRenderer {
    ...
    private static final String FS=String.join("\n",
            ...
            "varying vec2 vUv;uniform sampler2D uNoise;uniform float uTime;uniform float uSnow;uniform float uWind;uniform float uWindDir;uniform float uTurbulence;uniform float uSceneLight;uniform float uVisibility;uniform float uThermal;uniform float uDetail;",
            ...
            "void main(){vec2 p=vec2(vUv.x,1.0-vUv.y);float snow=clamp(uSnow,0.0,1.0);if(snow<0.004){gl_FragColor=vec4(0.0);return;}float detail=clamp(uDetail,0.5,1.0);float side=sin(uWindDir);float drift=side*(0.015+uWind*0.085);float gust=(0.002+uWind*0.010)*(0.60+0.70*uTurbulence);",
            " float far=flake(p+vec2(0.11,0.03),31.0,22.0,0.055+snow*0.025,2.9,0.110,0.24+snow*0.24,drift*0.52,0.035,gust*0.45);float mid=flake(p+vec2(0.29,0.13),21.0,16.0,0.083+snow*0.038,8.7,0.150,0.20+snow*0.34,drift*0.78,0.050,gust*0.72);float near=0.0;if(detail>0.62){near=flake(p+vec2(0.47,0.21),13.0,10.0,0.118+snow*0.052,15.4,0.210,0.15+snow*0.42,drift*1.10,0.070,gust);}",
    ...
    public void onSurfaceCreated(){
        ...
        uNoise=u("uNoise");uTime=u("uTime");uSnow=u("uSnow");uWind=u("uWind");uWindDir=u("uWindDir");uTurbulence=u("uTurbulence");uSceneLight=u("uSceneLight");uVisibility=u("uVisibility");uThermal=u("uThermal");uDetail=u("uDetail");
