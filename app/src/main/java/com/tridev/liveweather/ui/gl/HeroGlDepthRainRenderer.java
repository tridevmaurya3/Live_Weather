...
public final class HeroGlDepthRainRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "  float drizzleFine=rainBand(sceneP+vec2(0.13,0.04),vec2(104.0,62.0),0.46*windSpeed,0.0094,0.32,lean*0.38,2.3,0.17+drizzleGate*0.24,0.14);",
            "  float farRain=rainBand(sceneP+vec2(0.31,0.09),vec2(76.0,48.0),0.66*windSpeed,0.0120,0.42,lean*0.54,5.9,farGate,0.30);",
            "  float midRain=rainBand(sceneP+vec2(0.47,0.16),vec2(48.0,31.0),0.92*windSpeed,0.0168,0.58,lean*0.75,11.7,midGate,0.60);",
            "  float nearRain=0.0;",
            "  if(detail>0.62){",
            "    nearRain=rainBand(sceneP+vec2(0.67,0.25),vec2(27.0,18.0),1.18*windSpeed,0.0240,0.76,lean*0.96,18.1,nearGate,0.92);",
            "  }",
            "  float crossSpray=0.0;",
            "  if(detail>0.84&&uWind>0.58&&rain>0.50){",
            "    crossSpray=rainBand(sceneP+vec2(0.08,0.33),vec2(62.0,38.0),0.80*windSpeed,0.0104,0.40,lean*1.18+side*(0.014+uTurbulence*0.010),27.4,0.16+rain*0.16,0.46);",
            "  }",
