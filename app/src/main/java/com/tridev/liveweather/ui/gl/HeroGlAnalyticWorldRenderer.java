 ...
 *
 * Scenery S2 keeps Natural Hills as the compatibility baseline while making the
 * selectable Open Sky, Village, Farm, River/Lake, Flowers/Greenery and Urban packs
 * visually distinct. Scenery remains presentation-only: rain, fog, storm, Sun/Moon,
 * wetness and atmospheric light still come exclusively from the shared live snapshot.
 */
public final class HeroGlAnalyticWorldRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            " float windowShape=step(0.80,fract(cityCell*0.618))*step(0.22,cityLocal)*step(cityLocal,mUrban>0.5?0.55:0.41);",
            ...
            " float flowerA=step(0.955,0.5+0.5*sin(x*113.0+floor(p.y*112.0)*2.31));",
            " float flowerB=step(0.965,0.5+0.5*sin(x*79.0-floor(p.y*96.0)*1.73+1.4));",
