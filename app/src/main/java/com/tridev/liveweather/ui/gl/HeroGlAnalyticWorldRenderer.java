...
public final class HeroGlAnalyticWorldRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "uniform float uFog;",
            "uniform float uHaze;",
            "uniform float uFarTransmission;",
            "uniform float uMidTransmission;",
            "uniform float uNearTransmission;",
            "uniform float uMicroVisibility;",
            ...
            "uniform float uVegetationVitality;",
            "uniform float uVegetationDryStress;",
            "uniform float uVegetationColdStress;",
            ...
            "vec3 vegetationMaterial(vec3 base,float vitality,float dryStress,float coldStress){",
            " float luma=dot(base,vec3(0.299,0.587,0.114));",
            " vec3 dryTone=vec3(luma*1.10,luma*0.92,luma*0.48);",
            " vec3 coldTone=vec3(luma)*vec3(0.90,0.96,1.04);",
            " base=mix(base,dryTone,clamp(dryStress,0.0,1.0)*0.46);",
            " base=mix(base,coldTone,clamp(coldStress,0.0,1.0)*0.30);",
            " float lush=max(0.0,clamp(vitality,0.0,1.0)-0.70);",
            " return mix(base,base*vec3(0.94,1.07,0.96),lush*0.22);",
            "}",
            "void main(){",
            ...
            " float canopyLine=0.873+0.009*sin(TAU*(xNear*(4.7+vNorm*0.32))+0.4+vPhase*0.11)+0.006*sin(TAU*(xNear*8.9)+1.7-vPhase*0.09)+0.0035*sin(TAU*(xNear*15.4)+2.6)+0.0022*sin(uTime*(0.30+uWind*0.48)+xNear*18.0)*uWind;",
            ...
            " float vegetationVitality=clamp(uVegetationVitality,0.0,1.0);",
            " float vegetationDryStress=clamp(uVegetationDryStress,0.0,1.0);",
            " float vegetationColdStress=clamp(uVegetationColdStress,0.0,1.0);",
            ...
            " float farTransmission=clamp(uFarTransmission,0.08,1.0);",
            " float midTransmission=clamp(uMidTransmission,0.25,1.0);",
            " float nearTransmission=clamp(uNearTransmission,0.62,1.0);",
            " float forestTransmission=mix(midTransmission,nearTransmission,0.58);",
            " float structureTransmission=mix(midTransmission,nearTransmission,0.64);",
            " float microVisibility=clamp(uMicroVisibility,0.34,1.0);",
            " float detailVis=clamp((1.0-haze*0.86)*(0.52+light*0.48)*(1.0-uStorm*0.22)*(0.70+0.30*detailMid)*microVisibility,0.0,1.0);",
            ...
            " forestC=vegetationMaterial(forestC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            " vec3 color=vec3(0.0);float alpha=0.0;",
            " float a=farM*(0.42-haze*0.22)*farTransmission;color=mix(color,farC,a);alpha=max(alpha,a);",
            " a=midM*(0.58-haze*0.18)*midTransmission;color=mix(color,midC,a);alpha=max(alpha,a);",
            " a=max(forest*0.66,canopy*0.52)*forestTransmission;color=mix(color,forestC,a);alpha=max(alpha,a);",
            ...
            " color+=vec3(0.34,0.39,0.40)*farEdge*rimLight*farTransmission+vec3(0.20,0.27,0.28)*midEdge*rimLight*0.72*midTransmission;",
            " float solarTerrain=(farEdge*0.55*farTransmission+midEdge*midTransmission)*golden*solarSide*(1.0-haze*0.55);",
            " color+=goldenTone*solarTerrain*0.18;",
            " float moonTerrain=(farEdge*0.35*farTransmission+midEdge*0.55*midTransmission)*moonMaterial;",
            " color+=vec3(0.055,0.080,0.12)*moonTerrain*0.12;",
            " float terrainSnow=snowTruth*(farEdge*0.45*farTransmission+midEdge*0.72*midTransmission)*(1.0-uFog*0.55)*(1.0-openW);",
            ...
            " float ridgeDetail=mNature*midM*(1.0-nearM)*smoothstep(midLine+0.008,midLine+0.060,p.y)*detailVis*midTransmission;",
            ...
            " float legacySettlement=clamp((0.018+night*0.22+precip*0.055+uStorm*0.045)*(1.0-uFog*0.70)*mNature,0.0,0.40)*structureTransmission;",
            " float sceneStructure=designedScene*(0.46+light*0.24)*(1.0-uFog*0.68)*structureTransmission;",
            ...
            " hedgeC=vegetationMaterial(hedgeC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            ...
            " float windowBand=windowShape*buildingShape*night*(1.0-uFog*0.70)*(0.20*mUrban+0.13*mVillage+0.10*mNature)*structureTransmission;",
            " color+=vec3(0.78,0.58,0.30)*windowBand*0.55;alpha=max(alpha,windowBand*0.14);",
            " float roofEdge=mUrban*buildingShape*(1.0-smoothstep(cityTop+0.005,cityTop+0.014,p.y))*detailVis*structureTransmission;",
            ...
            " fieldBase=vegetationMaterial(fieldBase,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            ...
            " farmC=vegetationMaterial(farmC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            " farmC=mix(farmC,snowTone,snowTruth*0.10);",
            " color=mix(color,farmC,rowMask*(0.34+vNorm*0.04));alpha=max(alpha,rowMask*0.25);",
            " color+=vec3(0.10,0.12,0.045)*fineRows*farmGround*(0.026+vNorm*0.006)*(1.0-uFog*0.60)*microContrast*microVisibility;",
            ...
            " cropHeadC=vegetationMaterial(cropHeadC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            ...
            " bankC=vegetationMaterial(bankC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            " bankC=mix(bankC,snowTone,snowTruth*0.14);",
            " color=mix(color,bankC,bank*(0.45-haze*0.16));alpha=max(alpha,bank*0.40);",
            " float livingWater=clamp(mRiver+mNature*0.42+mFarm*0.16,0.0,1.0);",
            " float water=livingWater*smoothstep(0.885+riverShift,0.945,p.y);",
            ...
            " waterC+=vec3(0.025,0.038,0.050)*(wave*0.55+wave2*0.32+microWave*0.13*microContrast*microVisibility);",
            " waterC+=vec3(0.030,0.040,0.045)*rainDimple*precip*0.025*detailMid*microVisibility;",
            ...
            " reedC=vegetationMaterial(reedC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            ...
            " meadowC=vegetationMaterial(meadowC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            " meadowC=mix(meadowC,snowTone,snowTruth*0.09);",
            " color=mix(color,meadowC,meadow*(0.42+leafTexture*0.16*microContrast*microVisibility)*(1.0-uFog*0.42));alpha=max(alpha,meadow*0.46);",
            ...
            " grassC=vegetationMaterial(grassC,vegetationVitality,vegetationDryStress,vegetationColdStress);",
            ...
            " float flowerDots=meadow*max(flowerA,flowerB)*(1.0-uFog*0.72)*(0.34+light*0.44)*(0.55+0.45*detailMid)*microVisibility;",
            " flowerDots*=mix(0.34,1.0,vegetationVitality)*(1.0-vegetationDryStress*0.55)*(1.0-vegetationColdStress*0.62)*(1.0-snowTruth*0.70);",
            ...
            " float lifeVisibility=daylight*(1.0-uFog*0.82)*(1.0-uStorm*0.86)*(1.0-precip*0.70)*detailMid;",
            " float birdScene=clamp(mNature+mFarm+mRiver+mFlowers*0.75,0.0,1.0)*lifeVisibility;",
            " vec2 birdP=vec2(fract(screenP.x-uTime*(0.010+uWind*0.012)*windSign+0.18),screenP.y);",
            " vec2 birdA=(birdP-vec2(0.24,0.30+0.012*sin(uTime*0.55)))*vec2(aspect,1.0);",
            " vec2 birdB=(birdP-vec2(0.62,0.37+0.010*sin(uTime*0.43+1.7)))*vec2(aspect,1.0);",
            " float wingBeat=0.010+0.006*(0.5+0.5*sin(uTime*2.4));",
            " float birds=(1.0-smoothstep(0.0025,0.0065,abs(abs(birdA.y)-abs(birdA.x)*0.42-wingBeat)))*step(abs(birdA.x),0.028);",
            " birds=max(birds,(1.0-smoothstep(0.0025,0.0060,abs(abs(birdB.y)-abs(birdB.x)*0.38-wingBeat*0.78)))*step(abs(birdB.x),0.022));",
            " color=mix(color,mix(vec3(0.025,0.035,0.040),vec3(0.12,0.13,0.13),light),birds*birdScene*0.72);alpha=max(alpha,birds*birdScene*0.58);",
            " float turbineScene=clamp(mFarm+mNature*0.48+mVillage*0.25,0.0,1.0)*lifeVisibility;",
            " float turbineCell=floor(screenP.x*3.0);",
            " float turbineX=(turbineCell+0.52)/3.0+0.015*sin(turbineCell*2.1+vPhase);",
            " float turbineScale=0.78+0.18*sin(turbineCell*1.7+1.0);",
            " vec2 turbineP=vec2((screenP.x-turbineX)*aspect,screenP.y-(0.675-0.012*turbineScale));",
            " float tower=(1.0-smoothstep(0.0025,0.0060,abs(turbineP.x)))*step(0.0,turbineP.y)*step(turbineP.y,0.115*turbineScale);",
            " vec2 rotor=turbineP-vec2(0.0,0.002);",
            " float rotorRadius=length(rotor);",
            " float rotorAngle=atan(rotor.y,rotor.x)-uTime*(0.34+uWind*0.82)-turbineCell*0.7;",
            " float bladeAngle=abs(sin(rotorAngle*1.5));",
            " float blades=(1.0-smoothstep(0.10,0.23,bladeAngle))*step(0.010,rotorRadius)*step(rotorRadius,0.052*turbineScale);",
            " float hub=1.0-smoothstep(0.004,0.010,rotorRadius);",
            " float turbine=clamp(max(tower,max(blades,hub)),0.0,1.0)*turbineScene;",
            " vec3 turbineC=mix(vec3(0.12,0.15,0.16),vec3(0.68,0.72,0.72),light)*(1.0-deepNight*0.45);",
            " color=mix(color,turbineC,turbine*0.78);alpha=max(alpha,turbine*0.82);",
            " float animalScene=clamp(mNature+mFarm+mVillage*0.55+mRiver*0.35,0.0,1.0)*lifeVisibility;",
            " float animalX=fract(screenP.x-uTime*0.006*windSign+0.34);",
            " vec2 animalP=vec2((animalX-0.46)*aspect,screenP.y-0.795);",
            " float animalBody=1.0-smoothstep(0.85,1.0,length(animalP/vec2(0.030,0.012)));",
            " float animalHead=1.0-smoothstep(0.007,0.011,length(animalP-vec2(0.032,-0.006)));",
            " float animalLegs=(1.0-smoothstep(0.002,0.004,abs(abs(animalP.x)-0.017)))*step(0.004,animalP.y)*step(animalP.y,0.029);",
            " float animal=max(animalBody,max(animalHead,animalLegs))*animalScene;",
            " color=mix(color,mix(vec3(0.030,0.026,0.020),vec3(0.16,0.12,0.075),light),animal*0.74);alpha=max(alpha,animal*0.76);",
            ...
            " float terrainOcclusion=clamp(max(farM,midM),0.0,1.0)*(1.0-openW);",
            " float celestialTerrainMask=smoothstep(0.08,0.58,terrainOcclusion);",
            " vec2 sunDelta=(screenP-uSunPos)*vec2(aspect,1.0);",
            " float sunDistance=length(sunDelta);",
            " float sunDiscBlock=1.0-smoothstep(0.066,0.104,sunDistance);",
            " float sunHaloBlock=(1.0-smoothstep(0.100,0.250,sunDistance))*0.72;",
            " float sunTerrainBlock=max(sunDiscBlock,sunHaloBlock)*step(0.001,uSunVis);",
            " alpha=max(alpha,celestialTerrainMask*sunTerrainBlock);",
            " vec2 moonDelta=(screenP-uMoonPos)*vec2(aspect,1.0);",
            " float moonDistance=length(moonDelta);",
            " float moonDiscBlock=1.0-smoothstep(0.062,0.094,moonDistance);",
            " float moonHaloBlock=(1.0-smoothstep(0.090,0.210,moonDistance))*0.62;",
            " float moonTerrainBlock=max(moonDiscBlock,moonHaloBlock)*step(0.001,uMoonVis);",
    ...
    private final VegetationMaterialPolicy.Sample vegetationSample = new VegetationMaterialPolicy.Sample();
    private final AtmosphericPerspectivePolicy.Sample perspectiveSample = new AtmosphericPerspectivePolicy.Sample();
    ...
    private int uFog;
    private int uHaze;
    private int uFarTransmission;
    private int uMidTransmission;
    private int uNearTransmission;
    private int uMicroVisibility;
    private int uLight;
    ...
    private int uVegetationVitality;
    private int uVegetationDryStress;
    private int uVegetationColdStress;
    ...
    public void onSurfaceCreated() {
        ...
        uFog = u("uFog");
        uHaze = u("uHaze");
        uFarTransmission = u("uFarTransmission");
        uMidTransmission = u("uMidTransmission");
        uNearTransmission = u("uNearTransmission");
        uMicroVisibility = u("uMicroVisibility");
        ...
        uVegetationVitality = u("uVegetationVitality");
        uVegetationDryStress = u("uVegetationDryStress");
        uVegetationColdStress = u("uVegetationColdStress");
    ...
    public void drawFrame() {
        ...
        VegetationMaterialPolicy.resolveInto(
                vegetationSample,
                scene.environmentalMoisture,
                groundWetnessController.getSoilSaturation(),
                groundWetnessController.getWetness(),
                scene.rainIntensity,
                scene.drizzleIntensity,
                scene.snowIntensity,
                scene.thermalBias
        );
        AtmosphericPerspectivePolicy.resolveInto(perspectiveSample, scene.visibilityFactor);
        ...
        GLES20.glUniform1f(uFarTransmission, perspectiveSample.farTransmission);
        GLES20.glUniform1f(uMidTransmission, perspectiveSample.midTransmission);
        GLES20.glUniform1f(uNearTransmission, perspectiveSample.nearTransmission);
        GLES20.glUniform1f(uMicroVisibility, perspectiveSample.microVisibility);
        ...
        GLES20.glUniform1f(uVegetationVitality, vegetationSample.vitality);
        GLES20.glUniform1f(uVegetationDryStress, vegetationSample.dryStress);
        GLES20.glUniform1f(uVegetationColdStress, vegetationSample.coldStress);
