...
public final class HeroGlAnalyticWorldRenderer {
    ...
    private static final String FS=String.join("\n",
            ...
            " float treeCell=floor(x*28.0);float treeLocal=fract(x*28.0);float treeRnd=0.5+0.5*sin(treeCell*7.13+1.7);float treeTop=0.842+0.030*treeRnd;float treeHalf=0.105+0.045*treeRnd;",
            " float treeShape=1.0-smoothstep(0.070,treeHalf,abs(treeLocal-0.5));float treeMask=step(treeTop-(treeShape*0.050),p.y)*(1.0-smoothstep(0.920,0.945,p.y));",
            " float forestLine=0.878+0.009*sin(TAU*(x*6.1)+0.4)+0.006*sin(TAU*(x*10.8)+1.7);float farM=smoothstep(farLine-0.010,farLine+0.012,p.y);float midM=smoothstep(midLine-0.009,midLine+0.011,p.y);",
            " float nearM=smoothstep(nearLine-0.008,nearLine+0.010,p.y);float forest=smoothstep(forestLine-0.006,forestLine+0.008,p.y);float precip=max(uRain,uDrizzle*0.65);float night=1.0-smoothstep(-7.0,1.5,uSunAltitude);",
            " float lunar=night*uMoonVis*uMoonIllum*(1.0-uCloud*0.5);float light=clamp(0.18+uSceneLight*0.74+lunar*0.18,0.16,0.95);float haze=max(uFog*0.55,uHaze*0.25);float warm=max(0.0,uThermal);float cold=max(0.0,-uThermal);",
            ...
            " vec3 color=vec3(0.0);float alpha=0.0;float a=farM*(0.46-haze*0.18);color=mix(color,farC,a);alpha=max(alpha,a);a=midM*(0.65-haze*0.16);color=mix(color,midC,a);alpha=max(alpha,a);",
            " a=nearM*0.83;color=mix(color,nearC,a);alpha=max(alpha,a);a=max(forest*0.86,treeMask*0.82);color=mix(color,forestC,a);alpha=max(alpha,a);",
            " float ground=smoothstep(0.918,0.953,p.y);vec3 groundC=mix(vec3(0.008,0.018,0.024),vec3(0.025,0.046,0.052),light);groundC=mix(groundC,vec3(0.032,0.052,0.060),cold*0.045);color=mix(color,groundC,ground);alpha=max(alpha,ground);",
            " float settlement=clamp(0.12+night*0.30+precip*0.22+uStorm*0.20,0.0,0.58);float cityCell=floor(fract(x)*18.0);float cityLocal=fract(x*18.0);float cityTop=0.744+0.055*(0.5+0.5*sin(cityCell*2.07+0.9));",
            " float building=step(cityTop,p.y)*step(cityLocal,0.66)*(1.0-smoothstep(0.908,0.938,p.y))*settlement;vec3 cityC=mix(vec3(0.026,0.039,0.052),vec3(0.045,0.055,0.064),light);color=mix(color,cityC,building*0.72);alpha=max(alpha,building*0.72);",
            " float windowBand=step(0.80,fract(cityCell*0.618))*step(0.20,cityLocal)*step(cityLocal,0.48)*building*night*(1.0-uFog*0.65);color+=vec3(0.78,0.58,0.30)*windowBand*0.18;alpha=max(alpha,windowBand*0.12);",
            " float wet=smoothstep(0.16,0.74,precip)*ground;float reflectionBand=smoothstep(0.925,0.995,p.y);float ripple=0.5+0.5*sin(x*34.0+uTime*(0.75+uRain*1.35));float ripple2=0.5+0.5*sin(x*61.0-uTime*0.48+1.4);",
            " float wetSheen=wet*reflectionBand*(0.019+0.017*ripple+0.010*ripple2);vec3 reflected=mix(vec3(0.16,0.23,0.30),vec3(0.31,0.38,0.43),uSceneLight)*(0.65+uStorm*0.20);color=mix(color,reflected,clamp(wetSheen*3.4,0.0,0.18));",
            " float sunLow=smoothstep(0.60,0.94,uSunPos.y)*uSunVis*(1.0-night);float sunColumn=exp(-abs(p.x-uSunPos.x)*10.0)*reflectionBand*wet*sunLow*(0.010+0.025*ripple);",
            " float moonColumn=exp(-abs(p.x-uMoonPos.x)*13.0)*reflectionBand*wet*night*uMoonVis*uMoonIllum*(0.007+0.015*ripple2);color+=vec3(0.94,0.55,0.24)*sunColumn+vec3(0.42,0.58,0.78)*moonColumn;",
            " float lightReflection=windowBand*wet*reflectionBand*(0.025+0.035*ripple);color+=vec3(0.82,0.56,0.27)*lightReflection;alpha=max(alpha,ground);",
