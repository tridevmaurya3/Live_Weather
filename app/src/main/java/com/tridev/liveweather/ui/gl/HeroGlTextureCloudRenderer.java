...
public final class HeroGlTextureCloudRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "  vec3 rgb=texture2D(uAtlas,atlasUv).rgb;",
            "  float hi=max(rgb.r,max(rgb.g,rgb.b));",
            "  float lo=min(rgb.r,min(rgb.g,rgb.b));",
            "  float chroma=hi-lo;",
            "  float alpha=smoothstep(0.11,0.34,hi+chroma*0.18);",
            "  rgb=max(vec3(0.0),(rgb-vec3(0.075))/0.925);",
            ...
            "  float speed=0.006*(0.38+uWind*1.55);",
            "  float drift=wind.x*uTime*speed+(uParallax-0.5)*0.055;",
            "  float cell=uStorm>0.08?6.0:(uRain>0.06?5.0:(uCloud>0.78?4.0:(uCloud>0.52?3.0:(uCloud>0.25?2.0:1.0))));",
            "  float farCell=uCloud>0.68?4.0:(uCloud>0.32?3.0:0.0);",
            ...
            "  over(color,alpha,sprite(p,vec2(fract(0.33+drift*0.82),0.39),vec2(0.78,0.34),cell,(0.25+cover*0.43)*smoothstep(0.18,0.52,cover)),tint);",
            "  over(color,alpha,sprite(p,vec2(fract(0.86+drift*0.88),0.44),vec2(0.72,0.32),cell,(0.22+cover*0.40)*smoothstep(0.30,0.62,cover)),tint*0.94);",
            "  over(color,alpha,sprite(p,vec2(fract(0.55+drift*1.18),0.52),vec2(0.88,0.40),cell,(0.20+cover*0.48)*smoothstep(0.45,0.76,cover)),tint*0.88);",
            "  float overcast=smoothstep(0.76,0.94,cover);",
            "  float sheet=(0.14+0.16*sin(p.x*10.0+uTime*0.018)+0.08*sin(p.x*23.0-uTime*0.011))*overcast;",
            "  color=mix(color,vec3(0.43,0.48,0.54)*shade,clamp(sheet,0.0,0.26));",
            "  alpha=1.0-(1.0-alpha)*(1.0-clamp(sheet,0.0,0.26));",
            "  gl_FragColor=vec4(clamp(color,0.0,1.0),clamp(alpha,0.0,0.94));",
            "}");

    private final FloatBuffer quad;
    private int program,texture,aPosition,uAtlas,uResolution,uTime,uCloud,uDensity,uRain,uStorm,uBrightness,uWind,uWindDir,uParallax;
    ...
    public void onSurfaceCreated(){
        ...
        texture=loadTexture();
        aPosition=GLES20.glGetAttribLocation(program,"aPosition");uAtlas=u("uAtlas");uResolution=u("uResolution");
        uTime=u("uTime");uCloud=u("uCloud");uDensity=u("uDensity");uRain=u("uRain");uStorm=u("uStorm");
        uBrightness=u("uBrightness");uWind=u("uWind");uWindDir=u("uWindDir");uParallax=u("uParallax");
    ...
    public void drawFrame(){
        ...
        GLES20.glUniform1f(uDensity,s.cloudDensity);GLES20.glUniform1f(uRain,s.rainIntensity);
        GLES20.glUniform1f(uStorm,s.stormIntensity);GLES20.glUniform1f(uBrightness,s.cloudBrightness);
        GLES20.glUniform1f(uWind,s.windStrength);GLES20.glUniform1f(uWindDir,s.windDirectionRadians);GLES20.glUniform1f(uParallax,s.parallax);
    ...
    public void release(){if(texture!=0){int[] ids={texture};GLES20.glDeleteTextures(1,ids,0);texture=0;}if(program!=0){GLES20.glDeleteProgram(program);program=0;}}
