 ...
 *
 * Device-driven shape correction:
 * - cloud sprite dimensions stay compact but are visually thicker / puffier,
 * - procedural volume fill removes the hollow horizontal brush-stripe look,
 * - the atlas now supplies texture/detail instead of defining the entire body alpha,
 * - a soft compound-lobe mask removes pointed/cut-out atlas edges,
 * - subtle time-varying lobe growth makes clouds feel like they form/dissolve,
 * - more small masses preserve resolved cloud coverage without giant screen-wide blobs.
 *
 * Weather truth remains authoritative. Formation only redistributes the resolved
 * cloud field visually; it never creates cloud cover when the current scene is clear.
 */
public final class HeroGlTextureCloudRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "  return 1.0-smoothstep(0.78,1.08,d);",
            "}",
            "float roundedCloudMask(vec2 q,float seed,float evolve){",
            "  float lift=(evolve-0.5)*0.026;",
            "  float spread=(evolve-0.5)*0.018;",
            "  float base=ellipseMask(q,vec2(0.50,0.640),vec2(0.390+spread,0.255));",
            "  float belly=ellipseMask(q,vec2(0.50,0.745),vec2(0.310,0.165+evolve*0.012));",
            "  float left=ellipseMask(q,vec2(0.300-spread*0.25,0.595+lift),vec2(0.235,0.265+evolve*0.016));",
            "  float right=ellipseMask(q,vec2(0.700+spread*0.25,0.605-lift*0.30),vec2(0.230,0.255+evolve*0.015));",
            "  float crownA=ellipseMask(q,vec2(0.420,0.410-lift),vec2(0.245+evolve*0.015,0.315));",
            "  float crownB=ellipseMask(q,vec2(0.620,0.455+lift*0.30),vec2(0.215,0.275+evolve*0.018));",
            "  float crownC=ellipseMask(q,vec2(0.245+0.020*sin(seed),0.510),vec2(0.170,0.205));",
            "  float lobes=max(base,max(belly,max(left,max(right,max(crownA,max(crownB,crownC))))));",
            "  float feather=smoothstep(0.0,0.065,q.x)*smoothstep(0.0,0.065,1.0-q.x)*smoothstep(0.0,0.060,q.y)*smoothstep(0.0,0.060,1.0-q.y);",
            "  return clamp(lobes*feather,0.0,1.0);",
            "}",
            ...
            "  float evolve=0.5+0.5*sin(uTime*(0.038+0.006*mod(formSeed,3.0))+formSeed*1.731+cell*0.83);",
            "  vec2 q0=clamp(q,0.0,1.0);",
            "  vec2 q1=clamp(q+vec2(0.028,-0.020),0.0,1.0);",
            ...
            "  float formation=0.90+0.10*evolve;",
            "  float texDetail=clamp(max(s0.a,max(s1.a*0.90,s2.a*0.82)),0.0,1.0);",
            "  float volume=shape*(0.48+0.52*texDetail);",
            "  vec3 atlasRgb=(s0.rgb+s1.rgb+s2.rgb)/3.0;",
            "  float atlasLum=dot(atlasRgb,vec3(0.2126,0.7152,0.0722));",
            "  float underside=1.0-0.13*smoothstep(0.52,0.94,q.y);",
            "  vec3 neutral=vec3(0.74,0.77,0.81)*underside;",
            "  vec4 s=vec4(mix(neutral,atlasRgb,smoothstep(0.03,0.34,atlasLum)*0.62),inside*opacity*volume*formation);",
            "  return s;",
            "}",
            "void over(inout vec3 color,inout float alpha,vec4 s,vec3 tint){",
            "  float a=clamp(s.a,0.0,0.76);",
            ...
            "  float breatheA=sin(uTime*0.062+0.7)*0.0060;float breatheB=sin(uTime*0.043+2.2)*0.0050;",
            ...
            "  float farOpacity=(0.080+mass*0.225)*smoothstep(0.035,0.25,cover)*(0.16+0.84*farTruth);",
            "  float midOpacity=(0.130+mass*0.365)*smoothstep(0.12,0.48,cover)*(0.12+0.88*midTruth);",
            "  float nearOpacity=(0.115+mass*0.405)*smoothstep(0.28,0.72,cover)*(0.08+0.92*nearTruth);",
            "  float farDrift=drift*0.44;float midDrift=drift*0.80;float nearDrift=drift*1.15;",
            "  float formA=0.76+0.24*(0.5+0.5*sin(uTime*0.041+0.9));",
            "  float formB=0.76+0.24*(0.5+0.5*sin(uTime*0.035+2.4));",
            "  float formC=0.76+0.24*(0.5+0.5*sin(uTime*0.047+4.1));",
            "  vec3 color=vec3(0.0);float alpha=0.0;",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.12+farDrift),0.195+lift*0.16+breatheB),vec2(0.32,0.170),farCell,farOpacity*0.92,0.0,1.1),tint*1.10);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.48+farDrift*1.03),0.245-lift*0.12-breatheA),vec2(0.29,0.155),farAlt,farOpacity*0.66*formA,1.0,2.7),tint*1.065);",
            "  if(detail>0.66){over(color,alpha,spriteWrapped(p,vec2(fract(0.79+farDrift*1.08),0.285+breatheA*0.35),vec2(0.27,0.145),farCell,farOpacity*0.54*formB,0.0,4.2),tint*1.05);}",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.18+midDrift),0.345+lift*0.32+breatheA),vec2(0.36,0.240),cell,midOpacity*0.88,0.0,5.6),tint);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.57+midDrift*1.04),0.410-lift*0.25-breatheB),vec2(0.34,0.215),altCell,midOpacity*0.64*formB,1.0,7.3),tint*0.98);",
            "  if(detail>0.68){over(color,alpha,spriteWrapped(p,vec2(fract(0.88+midDrift*0.96),0.365+breatheB*0.45),vec2(0.30,0.195),cell,midOpacity*0.48*formC,0.0,8.8),tint*1.01);}",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.30+nearDrift),0.505+lift*0.45+breatheB),vec2(0.39,0.290),cell,nearOpacity*0.84,1.0,10.4),tint*0.92);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.72+nearDrift*1.02),0.470-lift*0.20+breatheA),vec2(0.34,0.255),altCell,nearOpacity*0.58*formA,0.0,12.1),tint*0.95);",
            "  if(detail>0.82&&nearTruth>0.34){over(color,alpha,spriteWrapped(p,vec2(fract(0.03+nearDrift*0.92),0.555+breatheB*0.45),vec2(0.30,0.215),cell,nearOpacity*0.42*formC,1.0,13.9),tint*0.94);}",
            "  if(detail>0.88&&cover<0.46&&farTruth>0.12){",
            "    float wispOpacity=(0.045+0.090*farTruth)*(1.0-smoothstep(0.34,0.52,cover));",
            "    over(color,alpha,spriteWrapped(p,vec2(fract(0.41+farDrift*1.26),0.315+breatheA*0.55),vec2(0.24,0.110),4.0,wispOpacity*formB,1.0,15.2),vec3(1.08)*tint);",
            "  }",
