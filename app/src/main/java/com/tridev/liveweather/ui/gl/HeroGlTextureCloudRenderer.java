 ...
 * - cloud sprite dimensions are roughly half of the previous oversized masses,
...
public final class HeroGlTextureCloudRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "  float lift=(evolve-0.5)*0.030;",
            "  float spread=(evolve-0.5)*0.020;",
            "  float base=ellipseMask(q,vec2(0.50,0.625),vec2(0.43+spread,0.205));",
            "  float left=ellipseMask(q,vec2(0.29-spread*0.35,0.575+lift),vec2(0.255,0.235+evolve*0.018));",
            "  float right=ellipseMask(q,vec2(0.71+spread*0.35,0.585-lift*0.35),vec2(0.250,0.225+evolve*0.015));",
            "  float crownA=ellipseMask(q,vec2(0.43,0.405-lift),vec2(0.260+evolve*0.018,0.275));",
            "  float crownB=ellipseMask(q,vec2(0.61,0.445+lift*0.35),vec2(0.225,0.235+evolve*0.020));",
            "  float crownC=ellipseMask(q,vec2(0.23+0.025*sin(seed),0.505),vec2(0.180,0.185));",
            "  float lobes=max(base,max(left,max(right,max(crownA,max(crownB,crownC)))));",
            "  float feather=smoothstep(0.0,0.055,q.x)*smoothstep(0.0,0.055,1.0-q.x)*smoothstep(0.0,0.050,q.y)*smoothstep(0.0,0.050,1.0-q.y);",
            ...
            "  float formation=0.88+0.12*evolve;",
            "  s.a*=inside*opacity*shape*formation;",
            "  return s;",
            "}",
            ...
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.12+farDrift),0.195+lift*0.16+breatheB),vec2(0.38,0.130),farCell,farOpacity*0.92,0.0,1.1),tint*1.10);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.48+farDrift*1.03),0.245-lift*0.12-breatheA),vec2(0.34,0.120),farAlt,farOpacity*0.66*formA,1.0,2.7),tint*1.065);",
            "  if(detail>0.66){over(color,alpha,spriteWrapped(p,vec2(fract(0.79+farDrift*1.08),0.285+breatheA*0.35),vec2(0.31,0.112),farCell,farOpacity*0.54*formB,0.0,4.2),tint*1.05);}",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.18+midDrift),0.345+lift*0.32+breatheA),vec2(0.43,0.185),cell,midOpacity*0.88,0.0,5.6),tint);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.57+midDrift*1.04),0.410-lift*0.25-breatheB),vec2(0.39,0.170),altCell,midOpacity*0.64*formB,1.0,7.3),tint*0.98);",
            "  if(detail>0.68){over(color,alpha,spriteWrapped(p,vec2(fract(0.88+midDrift*0.96),0.365+breatheB*0.45),vec2(0.34,0.150),cell,midOpacity*0.48*formC,0.0,8.8),tint*1.01);}",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.30+nearDrift),0.505+lift*0.45+breatheB),vec2(0.46,0.215),cell,nearOpacity*0.84,1.0,10.4),tint*0.92);",
            "  over(color,alpha,spriteWrapped(p,vec2(fract(0.72+nearDrift*1.02),0.470-lift*0.20+breatheA),vec2(0.40,0.190),altCell,nearOpacity*0.58*formA,0.0,12.1),tint*0.95);",
            "  if(detail>0.82&&nearTruth>0.34){over(color,alpha,spriteWrapped(p,vec2(fract(0.03+nearDrift*0.92),0.555+breatheB*0.45),vec2(0.34,0.160),cell,nearOpacity*0.42*formC,1.0,13.9),tint*0.94);}",
            "  if(detail>0.88&&cover<0.46&&farTruth>0.12){",
            "    float wispOpacity=(0.045+0.090*farTruth)*(1.0-smoothstep(0.34,0.52,cover));",
            "    over(color,alpha,spriteWrapped(p,vec2(fract(0.41+farDrift*1.26),0.315+breatheA*0.55),vec2(0.27,0.085),4.0,wispOpacity*formB,1.0,15.2),vec3(1.08)*tint);",
            "  }",
            ...
            "  float sheet=(0.095+0.050*xWave+0.030*xWave2)*ceilingStrength*ceiling;",
            "  vec3 sheetTint=mix(vec3(0.56,0.60,0.64),vec3(0.38,0.42,0.48),clamp(uStorm*0.75+uRain*0.15+ceilingTruth*0.16,0.0,1.0))*shade;",
            "  color=mix(color,sheetTint,clamp(sheet,0.0,0.22));alpha=1.0-(1.0-alpha)*(1.0-clamp(sheet,0.0,0.22));",
            "  float undersideBand=smoothstep(0.40,0.65,p.y)*(1.0-smoothstep(0.77,0.95,p.y));",
            "  float underside=undersideBand*ceilingStrength*(0.010+weatherShade*0.027+ceilingTruth*0.017);",
            "  color=mix(color,vec3(0.22,0.26,0.32),underside);alpha=1.0-(1.0-alpha)*(1.0-underside*0.42);",
