 ...
 *
 * Video-driven cloud sculpting:
 * - compact sprites stay thick but no longer read as smooth oval/pancake discs,
 * - irregular lobes build a cauliflower-like upper body and uneven soft base,
 * - procedural volume keeps interiors continuous while atlas data supplies texture only,
 * - subtle contour warp makes cloud masses form/dissolve without pulsing,
...
public final class HeroGlTextureCloudRenderer {
    ...
    private static final String FS = String.join("\n",
            ...
            "  float lift=(evolve-0.5)*0.024;",
            "  float wobble=0.014*sin(q.y*13.0+seed*1.7)+0.009*sin(q.y*23.0-seed);",
            "  vec2 wq=vec2(q.x+wobble,q.y);",
            "  float bL=ellipseMask(wq,vec2(0.28,0.675+lift*0.20),vec2(0.205,0.205));",
            "  float bM=ellipseMask(wq,vec2(0.49,0.700-lift*0.15),vec2(0.245,0.220));",
            "  float bR=ellipseMask(wq,vec2(0.72,0.665+lift*0.10),vec2(0.205,0.205));",
            "  float cL=ellipseMask(wq,vec2(0.245,0.520+lift),vec2(0.175,0.235));",
            "  float cM=ellipseMask(wq,vec2(0.455,0.405-lift*0.60),vec2(0.225,0.305));",
            "  float cR=ellipseMask(wq,vec2(0.655,0.475+lift*0.25),vec2(0.195,0.270));",
            "  float top=ellipseMask(wq,vec2(0.500+0.025*sin(seed),0.300-lift*0.45),vec2(0.150,0.185));",
            "  float side=ellipseMask(wq,vec2(0.820,0.565-lift*0.10),vec2(0.115,0.160));",
            "  float lobes=max(max(bL,max(bM,bR)),max(max(cL,max(cM,cR)),max(top,side)));",
            "  float baseRipple=0.018*sin(q.x*18.0+seed*2.3)+0.010*sin(q.x*31.0-seed);",
            "  float lowerFade=1.0-smoothstep(0.83+baseRipple,0.985+baseRipple,q.y);",
            "  float feather=smoothstep(0.0,0.060,q.x)*smoothstep(0.0,0.060,1.0-q.x)*smoothstep(0.0,0.055,q.y)*lowerFade;",
            ...
            "  float evolve=0.5+0.5*sin(uTime*(0.032+0.004*mod(formSeed,3.0))+formSeed*1.731+cell*0.83);",
            "  vec2 q0=clamp(q,0.0,1.0);",
            "  vec2 q1=clamp(q+vec2(0.031,-0.023),0.0,1.0);",
            "  vec2 q2=clamp(q+vec2(-0.027,0.029),0.0,1.0);",
            ...
            "  float formation=0.93+0.07*evolve;",
            "  float texDetail=clamp((s0.a+s1.a+s2.a)/2.15,0.0,1.0);",
            "  float volume=shape*(0.72+0.28*texDetail);",
            "  vec3 atlasRgb=(s0.rgb+s1.rgb+s2.rgb)/3.0;",
            "  float atlasLum=dot(atlasRgb,vec3(0.2126,0.7152,0.0722));",
            "  float micro=0.5+0.5*sin(q.x*19.0+q.y*13.0+formSeed)*sin(q.x*11.0-q.y*17.0+cell);",
            "  float topLight=1.08-0.20*smoothstep(0.28,0.92,q.y);",
            "  float textureTone=mix(0.92,1.08,clamp(atlasLum*0.72+micro*0.28,0.0,1.0));",
            "  vec3 neutral=vec3(0.74,0.77,0.81)*topLight*textureTone;",
            "  vec3 detailRgb=mix(neutral,atlasRgb,0.22+0.18*texDetail);",
