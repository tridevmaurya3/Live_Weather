...
public final class LiveWeatherWallpaperService extends WallpaperService {
    ...
    @Override public Engine onCreateEngine(){return new LiveWeatherEngine();}
    ...
    private final class LiveWeatherEngine extends Engine {
        ...
        @Nullable private WallpaperPreferences.Options appliedOptions;
        ...
        @Override public void onVisibilityChanged(boolean value){
            visible=value;handler.removeCallbacks(cacheRefreshRunnable);glRenderer.setVisible(value);
            if(value){reloadCache();handler.postDelayed(cacheRefreshRunnable,CACHE_RELOAD_MILLIS);}
        }

        @Override public void onSurfaceChanged(@NonNull SurfaceHolder holder,int format,int width,int height){
            super.onSurfaceChanged(holder,format,width,height);glRenderer.attachSurface(holder.getSurface(),width,height);glRenderer.setVisible(visible);
        }

        @Override public void onSurfaceDestroyed(@NonNull SurfaceHolder holder){glRenderer.detachSurface();super.onSurfaceDestroyed(holder);}
        ...
        private void reloadCache(){
            ...
            glRenderer.setWeatherData(weather.getWeather(),airResponse,weather.getLatitude(),weather.getLongitude());
            appliedWeatherSavedAt=weatherSavedAt;appliedAirSavedAt=airSavedAt;appliedLatitude=weather.getLatitude();appliedLongitude=weather.getLongitude();
        }
        ...
        private void applyOptions(@NonNull WallpaperPreferences.Options newOptions){
            if(appliedOptions==null||!sameOptions(appliedOptions,newOptions)){
                glRenderer.setVisualOptions(newOptions);appliedOptions=newOptions;
            }
            ...
            CinematicPerformanceGovernor.Profile profile=CinematicPerformanceGovernor.resolve(
                    ...
                    CinematicPerformanceGovernor.Surface.LIVE_WALLPAPER
            );
            glRenderer.setPerformanceProfile(profile.frameIntervalMillis,profile.detailScale);
        }

        private void resetAppliedWeatherIdentity(){
            appliedWeatherSavedAt=NO_CACHE_VERSION;appliedAirSavedAt=NO_CACHE_VERSION;appliedLatitude=Double.NaN;appliedLongitude=Double.NaN;
        }
        ...
        private boolean sameCoordinate(double first,double second){
            if(Double.isNaN(first)||Double.isNaN(second))return false;return Math.abs(first-second)<0.000001d;
        }
    }
}
